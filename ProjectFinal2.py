...
def save_to_file(date: str):
    ...
    with open(filename, 'a') as file:  # 'a' mode to append if file exists
...
# 6. Function to Read and Display Existing Data from a File
def read_existing_data(date: str):
    """Check if a file exists for the given date, and read and display the data if it exists."""
    filename = f"data_{date}.txt"
    if os.path.exists(filename):
        print(f"\nData for {date} already exists. Displaying previous data:\n")
        with open(filename, 'r') as file:
            print(file.read())  # Display the content of the file
    else:
...
if __name__ == "__main__":
    ...
    # Check if there is existing data for the entered date and display it
    read_existing_data(session_date)
